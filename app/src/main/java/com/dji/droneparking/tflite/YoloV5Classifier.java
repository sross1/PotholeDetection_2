...
//                    options.setAllowBufferHandleOutput(true);
        ...
        //calculates the middle value output of the tflite model, e.g. the 10647 value in the output float32[1,10647,7], which was viewed on netron.app
        d.output_box = (int) ((Math.pow((inputSize / 32), 2) + Math.pow((inputSize / 16), 2) + Math.pow((inputSize / 8), 2)) * 3);
        Log.d("Yolov5 classifier", "create: " + d.output_box);
...
//        d.ANCHORS = anchors;
        ...
        /*numClass - getting the number of classes from the model itself is wrong for some reason. It was showing 7 classes when at the
        moment there are only two, marestail and pigweed. Hardcoded the value but it can be read from labels file and passed into class.*/
        //int numClass = shape[shape.length - 1];
        int numClass = 2;
        Log.d("Numclass", "create: " + numClass);
        d.numClass = numClass;
...
//        int[] intValues = new int[INPUT_SIZE * INPUT_SIZE];
    ...
    public ArrayList<Recognition> recognizeImage(Bitmap bitmap) {
        Log.d("YoloV5Classifier", "recognizeImage: Bitmap Size: h: " + bitmap.getHeight() + " w: " + bitmap.getWidth());
...
//        final ArrayList<Recognition> recognitions = detections;
        Log.d("Yolov5", "recognizeImage: Recog size " + recognitions.size());
        return recognitions;
    }
